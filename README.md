...
- "Parcele oko mene": every parcel within 150 m of the GPS fix drawn as vector outlines with numbers; tap one to select it. This reuses the spatial `circle` request the GeoSrbija map viewer itself sends on a click (`st:"circle", s:"E,N,R", layers:"586,939,"`), so it needs no token or account. The place line for such parcels is the Latin half of the service's `desc` (cadastral municipality and municipality together).
- Opening a searched or saved parcel also loads its neighboring boundaries, centered on the parcel geometry without needing GPS. The query covers the parcel bounding circle plus 150 m, capped at the service limit of 1 km with an explicit partial-coverage label for larger parcels. Clearing selection retains this area; “Parcele oko mene” returns to GPS. Late responses cannot replace another area.
