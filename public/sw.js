...
const CACHE='parcele-shell-v9';
const SHELL=['/teren.html','/teren.css','/teren.js','/config.js','/satellite.js','/nearby-loader.js','/location-tracker.js','/parcel-neighborhood.js','/geo.js','/geosrbija.js','/field-geo.js','/favicon.svg','/icon-192.png','/icon-512.png','/manifest.webmanifest'];
...
self.addEventListener('message',e=>{if(e.data==='CHECK_SHELL')e.waitUntil((async()=>{const c=await caches.open(CACHE),ok=(await Promise.all(SHELL.map(p=>c.match(p)))).every(Boolean);e.ports[0]?.postMessage({ok});})());});
