import test from 'node:test';import assert from 'node:assert/strict';import fs from 'node:fs';import vm from 'node:vm';import {boundary,bearing,distance,unlocal} from '../public/field-geo.js';import {parcelGeometry} from '../public/geo.js';
...
test('sample has exact eight vertices and plausible area',()=>{const s=JSON.parse(fs.readFileSync('public/sample.json')),g=parcelGeometry(s.record);assert.equal(g.points.length,8);assert(g.area>1000&&g.area<4000);assert(Math.abs(g.points[0].lat-44.3374173087)<1e-8);});
